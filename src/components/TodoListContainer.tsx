...
export default function TodoListContainer() {
  ...
  const changeInDatabase = () => {
    // if the list type is completed then find the changed todos and update db.json for each changed todo using fetch
    // if the list type is open then find the changed todos and update db.json for each changed todo using fetch
    // recall the getData function to update the todoList
    if (listType === "completed") {
      const changedTodos = completedTodos.filter(
      ...
      const changedTodos = openTodos.filter((item) => item.completed === true);
      changedTodos.forEach((item) => {
        fetch(`http://localhost:3030/todo/${item.id}`, {
          method: "PATCH",
          headers: {
